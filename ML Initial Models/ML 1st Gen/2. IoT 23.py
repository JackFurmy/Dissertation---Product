...
TRAIN_DATA_PATH = (
    "/Users/jackfurmston/developer/cybersecurity_game/data/Network Traffic Data/"
    "Institutional/IoT-23/Malicous and Benign/phishing_40p2.csv"
)

HISTORICAL_FILE = (
    "/Users/jackfurmston/developer/cybersecurity_game/models/"
    "1. Baseline Offline Model/Phishing Websites:Emails/Phishing_historical.xlsx"
)
...
LEAKY_COLUMNS = [
    "detailed-label",
    "ts",
    "id.orig_h_oct1","id.orig_h_oct2","id.orig_h_oct3","id.orig_h_oct4","conn_state", "orig_bytes",
    "id.resp_h_oct1","id.resp_h_oct2","id.resp_h_oct3","id.resp_h_oct4", "duration", "id.resp_p"
]
...
def load_data(file_path, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    ...
    if label_col not in df.columns:
        print(f"[ERROR] Label column '{label_col}' missing. Exiting.")
        return None, None
...
def plot_mutual_info(X, y):
    ...
    plt.title("Feature 'Leakage' Score - IoT-23 Dataset")
    plt.tight_layout()
    plt.show()
...
def train_and_evaluate_models(
    ...
    models = {
        ...
        "RF" : RandomForestClassifier(n_estimators=50, max_depth=5, min_samples_leaf=50, random_state=42),
...
def main():
    X, y = load_data(TRAIN_DATA_PATH, label_col="label", drop_leak=LEAKY_COLUMNS)
    ...
    print(f"[INFO] Stratified split => Train={X_train.shape[0]}, Val={X_val.shape[0]}")
