...
TRAIN_DATA_PATH = "cybersecurity_game/data/Network Traffic Data/Institutional/CICIDS 2017/Traffic Label/train_10p.csv"
HISTORICAL_FILE = "cybersecurity_game/models/1. Baseline Offline Model/Network Traffic Data (multi-attack)/cicids2017_historical.xlsx"
...
LEAKY_COLUMNS = [
    ...
    "source ip_oct1","source ip_oct2","source ip_oct3","source ip_oct4",
    "destination ip_oct1","destination ip_oct2","destination ip_oct3","destination ip_oct4","packet length variance", "packet length mean", 
   ...
   "fwd packet length max", "fwd packet length mean",
    ...
    "bwd iat total",
    "bwd iat mean",
    "flow bytes/s", "flow iat mean", "flow iat max", "flow iat min", "flow iat std",
...
def load_data(file_path, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    ...
    after = len(df)
    print(f"[INFO] Removed {before - after} exact duplicates. Now {after} rows.")
...
def plot_mutual_info(X, y):
    ...
    plt.ylabel("Mutual Information w.r.t. label")
    plt.title("Feature 'Leakage' Score via MI")
    plt.tight_layout()
    plt.show()
    ...
    for name, clf in models.items():
        ...
        if name in partial_fit_models:
            for epoch in range(n_epochs):
                ...
                print(f"[Epoch {epoch+1}] {name} => Val Accuracy: {acc:.4f}, F1: {f1m:.4f}")

            y_pred = clf.predict(X_val_sc)
            y_proba= clf.predict_proba(X_val_sc) if hasattr(clf, "predict_proba") else None
            compute_metrics_multiclass(y_val_np, y_pred, y_proba)
...
def compute_metrics_multiclass(y_true, y_pred, y_proba=None):
    ...
    if y_proba is not None and len(np.unique(y_true)) == 2 and y_proba.shape[1] == 2:
        y_proba_attack = y_proba[:,1]
        if not np.isnan(y_proba_attack).any():
            rocAUC = roc_auc_score(y_true, y_proba_attack)
            prAUC  = average_precision_score(y_true, y_proba_attack)
            print(f"ROC AUC (binary): {rocAUC:.4f}")
            print(f"PR AUC (binary):  {prAUC:.4f}")

    y_true_bin = np.where(np.array(y_true) == 0, 0, 1)
...
def batch_retraining_demo():
    ...
    if LABEL_COL not in df_hist.columns:
        print(f"[WARN] '{LABEL_COL}' not found in historical file. Skipping.")
        return

    df_hist[LABEL_COL] = pd.to_numeric(df_hist[LABEL_COL], errors='coerce')
    df_hist.dropna(subset=[LABEL_COL], inplace=True)
    if df_hist.empty:
        print("[WARN] Historical file has no valid labeled rows. Skipping.")
        return
