...
TRAIN_DATA_PATH = (
    "cybersecurity_game/data/Malware Attack Patterns/Non-Institutional/EMBER/extracted/ember_2017_2/data_optimization/train_features_2_cleaned_step3_fe_optimized.csv"
)


HISTORICAL_FILE = TRAIN_DATA_PATH
...
def load_data(file_path, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    ...
    df = df[valid_mask]
    print(f"[INFO] After unifying -1->1 and keeping only 0/1, we have {df.shape[0]} rows left.")
...
def batch_retraining_demo():
    ...
    df_hist = pd.read_csv(HISTORICAL_FILE, low_memory=True)
    ...
    df_hist[LABEL_COL] = df_hist[LABEL_COL].replace({-1: 1})
    df_hist = df_hist[df_hist[LABEL_COL].isin([0,1])]
...
def main():
    ...
    if len(np.unique(y)) < 2:
        print("[ERROR] Only one class remains after filtering. Cannot do train_test_split with stratify.")
        return
