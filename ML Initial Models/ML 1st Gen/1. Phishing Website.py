...
TRAIN_DATA_PATH = (
    "cybersecurity_game/data/Phishing Websites/Phishing Websites/Training Dataset_converted_optimized.csv"
)

HISTORICAL_FILE = (
    "cybersecurity_game/models/1. Baseline Offline Model/Phishing Websites:Emails/Phishing_historical.xlsx"
)
...
def load_data(file_path, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    ...
    if label_col in df.columns:
        df[label_col] = df[label_col].replace({-1: 0, 1: 1})
    else:
        print(f"[ERROR] label_col '{label_col}' not found in DF.")
        return None, None
    ...
    if label_col not in df.columns:
        print(f"[ERROR] Label col '{label_col}' missing after dropping. Exiting.")
        return None, None
...
def plot_mutual_info(X, y):
    ...
    if X_num.empty:
        print("[WARN] No numeric columns for MI.")
        return None
    ...
    plt.title("Feature 'Leakage' Score - Phishing Data")
    plt.tight_layout()
    plt.show()
...
def batch_retraining_demo():
    ...
    df_hist[LABEL_COL] = df_hist[LABEL_COL].replace({-1:0, 1:1})
