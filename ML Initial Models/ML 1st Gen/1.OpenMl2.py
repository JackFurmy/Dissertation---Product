...
TRAIN_DATA_PATH = (
    "cybersecurity_game/data/Concept Drift and Adaptive Learning/OpenML/CovPokElec_converted.csv"
)
HISTORICAL_FILE = (
...
def plot_mutual_info(X, y):
    ...
    plt.ylabel("Mutual Information (label)")
    plt.title("Feature 'Leakage' Score via MI - CovPokElec")
    plt.tight_layout()
    plt.show()
...
def batch_retraining_demo():
    ...
    clf.fit(X_h, y_h)
    print("[INFO] Batch retraining complete with new historical data.")
...
def main():
    ...
    counts = Counter(y)
    rare_classes = [c for c, cnt in counts.items() if cnt < 2]
    if rare_classes:
        print(f"[WARN] Removing classes with <2 samples: {rare_classes}")
        mask = ~y.isin(rare_classes)
        X, y = X[mask], y[mask]
        print(f"[INFO] After removing rare classes => X={X.shape}, y={y.shape}")
