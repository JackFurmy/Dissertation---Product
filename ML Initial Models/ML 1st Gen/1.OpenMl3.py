...
TRAIN_DATA_PATH = (
    "cybersecurity_game/data/Concept Drift and Adaptive Learning/OpenML/gas_converted.csv"
)

HISTORICAL_FILE = (
    "cybersecurity_game/models/1. Baseline Offline Model/Concept Drift: Adatpvie Learning/concept Drift_historical.xlsx"
)
...
def load_data(file_path, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    ...
    if label_col not in df.columns:
        print(f"[ERROR] Label column '{label_col}' not found after dropping. Exiting.")
        return None, None
    ...
    after = len(df)
    print(f"[INFO] Removed {before - after} exact duplicates. Now {after} rows remain.")
...
def plot_mutual_info(X, y):
    ...
    if X_num.empty:
        print("[WARN] No numeric columns found. Skipping MI.")
        return None
    ...
    plt.title("Feature 'Leakage' Score via MI - Gas Dataset")
    plt.tight_layout()
    plt.show()
    ...
    for name, clf in models.items():
        ...
        if name in partial_fit_models:
            for epoch in range(n_epochs):
                ...
                print(f"[Epoch {epoch+1}] {name} => Accuracy: {acc:.4f}, Macro-F1: {macro_f1:.4f}")
...
def compute_metrics(y_true, y_pred, y_proba=None):
    ...
    if y_proba is not None:
        ...
        if len(unique_labels) == 2 and y_proba.shape[1] == 2:
            y_prob1 = y_proba[:,1]
...
def main():
    ...
    print(f"[INFO] Unique labels before shift: {sorted(y.unique())}")
    y = y - 1  
    print(f"[INFO] Unique labels after shift: {sorted(y.unique())}")

    from collections import Counter
    counts = Counter(y)
    rare = [c for c,cnt in counts.items() if cnt < 2]
    if rare:
        print(f"[WARN] Removing classes with <2 samples: {rare}")
        mask = ~y.isin(rare)
        X, y = X[mask], y[mask]
        print(f"[INFO] After removing => X={X.shape}, y={y.shape}")
