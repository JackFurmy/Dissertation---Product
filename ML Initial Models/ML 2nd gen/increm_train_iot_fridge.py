...
def stream_rows(csv_url: str, chunk: int, label: str = "label"):
    import pandas as pd

    print("[INFO] downloading IoT‑Fridge CSV …")
    df = (
    ...
    for start in range(0, len(df), chunk):
        for _, row in df.iloc[start : start + chunk].iterrows():
            y = int(row[label])
            x = row.drop(labels=[label, "timestamp", "type"], errors="ignore").to_dict()
            yield x, y
...
def bundle_path() -> Path:
    return MODEL_DIR / "iot_fridge.pkl"
...
def train_iot_fridge(
    chunk_size: int = 50_000,
    load_existing: bool = False,
    split_models: bool = False,
    user_id: str = "fridge_default_user",
):
    names = ["HoeffdingTree", "LogisticRegression", "NaiveBayes"]

    prev_models = {}
    if load_existing and bundle_path().exists():
        data = joblib.load(bundle_path())
        prev_models = {n: data["models"][n]["model"] for n in names}
    ...
    acc = {n: metrics.Accuracy() for n in names}
    cm  = {n: ConfusionMatrix()  for n in names}

    feats, rows = set(), 0

    for x_raw, y in stream_rows(IOT_FRIDGE_FIREBASE_URL, chunk_size):
        rows += 1
        x, feats = unify_feats(x_raw, feats)
    ...
    joblib.dump(
        {
            ...
            "best_name": best_name,
            "best_f1": best_f1,
            "dataset_url": IOT_FRIDGE_FIREBASE_URL,
        },
        bundle_path(),
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--user-id", default="fridge_default_user")
    args = ap.parse_args()
