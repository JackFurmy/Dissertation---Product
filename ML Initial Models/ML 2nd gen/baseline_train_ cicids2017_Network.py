...
BASELINE_MODEL_PATH = (
    "cybersecurity_game/game/game/models/models/cicids2017_baseline.pkl"
)

MI_FIG_DIR = "cybersecurity_game/game/game/mi figure"
MI_FIG_NAME= "cicids2017_mi.png"


def load_data_via_requests(firebase_url, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    print(f"[INFO] Downloading CSV from {firebase_url} ...")
...
def plot_mutual_info(X, y):
    ...
    print(f"[INFO] Computing mutual info for {X_num.shape[1]} numeric columns...")
    ...
    plt.title("CICIDS2017: Feature MI")
    plt.tight_layout()
...
def compute_metrics_binary(y_true, y_pred, y_proba=None):
    ...
    cm_bin = confusion_matrix((y_true==1).astype(int), (y_pred==1).astype(int), labels=[0,1])
    ...
    return acc, macro_f1, rocAUC, prAUC, tnr, far
...
def gather_model_metrics(clf, X_val_sc, y_val_np):
    ...
    cm_bin = confusion_matrix((y_val_np==1).astype(int), (y_pred==1).astype(int), labels=[0,1])
    tn, fp, fn, tp = cm_bin.ravel()
    tnr = tn/(tn+fp) if (tn+fp)>0 else 0
    far = fp/(tn+fp) if (tn+fp)>0 else 0
...
def train_and_evaluate_models(X_train, y_train, X_val, y_val, partial_fit_models=PARTIAL_FIT_MODELS,batch_size=BATCH_SIZE, n_epochs=N_EPOCHS):
    ...
    for name, clf in models.items():
        ...
        acc, f1m, rocA, prA, tnr, far = compute_metrics_binary(y_val_np, clf.predict(X_val_sc), y_proba)
...
def main():
    X, y = load_data_via_requests(FIREBASE_CICIDS2017_URL)
    ...
    all_trained = train_and_evaluate_models(
    ...
    best_name = None
    best_acc  = -1
    for name, info in all_trained.items():
        scoreboard = info["metrics"] 
        acc        = scoreboard["accuracy"]
        if acc > best_acc:
            best_acc  = acc
            best_name = name

    print(f"[INFO] Best model => {best_name}, accuracy={best_acc:.4f}")

    data_dict = {
        "models": all_trained,           
        "best_name": best_name,      
        "best_acc": best_acc,         
        "dataset_url": FIREBASE_CICIDS2017_URL,  
    }

    data_dict["columns"] = list(X_train.columns)
    data_dict["baseline_acc"] = best_acc
    data_dict["metrics"] = all_trained[best_name]["metrics"] 
