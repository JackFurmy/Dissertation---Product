...
def stream_rows(
    ...
    for url in url_list:
        print(f"[INFO] downloading ⇒ {url.split('/')[-1][:60]}…")
        df = (
...
def bundle_path() -> Path:
    return MODEL_DIR / "linux_memory.pkl"
...
def train_linux_memory(
    chunk_size: int = 50_000,
    load_existing: bool = False,
    split_models: bool = False,
    user_id: str = "linux_memory_user",
):
    prev_models = {}
    ...
    for x_raw, y in stream_rows(LINUX_MEMORY_URLS, chunk_size, label_encoders):
        rows += 1
    ...
    joblib.dump(
        {
            ...
            "best_name":   best_name,
            "best_f1":     best_f1,
            "dataset_urls": LINUX_MEMORY_URLS,
        },
        bundle_path(),
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--user-id",       default="linux_memory_user")
    args = ap.parse_args()
