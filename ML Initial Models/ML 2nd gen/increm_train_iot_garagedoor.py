...
def label_encode_value(col: str, val, encoders: Dict[str, Dict[str, int]]):
    ...
    val_str = str(val)
    if val_str not in enc:
        enc[val_str] = len(enc) + 1
    return float(enc[val_str])
...
def stream_rows(
    ...
    print("[INFO] downloading IoT‑Garage‑Door CSV …")
    df = (
    ...
    for start in range(0, len(df), chunk):
        for _, row in df.iloc[start : start + chunk].iterrows():
            y = int(row[label])
            raw = row.drop(labels=[label, "timestamp", "type"], errors="ignore").to_dict()
            x_enc = {c: label_encode_value(c, v, encoders) for c, v in raw.items()}
            yield x_enc, y
...
def bundle_path() -> Path:
    return MODEL_DIR / "iot_garage_door.pkl"
...
def train_iot_garage_door(
    chunk_size: int = 50_000,
    load_existing: bool = False,
    split_models: bool = False,
    user_id: str = "garage_default_user",
):
    prev_models = {}
    ...
    for x_raw, y in stream_rows(IOT_GARAGE_FIREBASE_URL, chunk_size, label_encoders):
        rows += 1
    ...
    joblib.dump(
        {
            ...
            "best_name": best_name,
            "best_f1": best_f1,
            "dataset_url": IOT_GARAGE_FIREBASE_URL,
        },
        bundle_path(),
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--user-id", default="garage_default_user")
    args = ap.parse_args()

    train_iot_garage_door(
