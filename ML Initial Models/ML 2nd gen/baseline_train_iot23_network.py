...
SAVE_BASELINE = True
BASELINE_MODEL_PATH = (
    "cybersecurity_game/game/game/models/models/iot23_baseline.pkl"
)

MI_FIG_DIR  = "cybersecurity_game/game/game/mi figure"
MI_FIG_NAME = "iot23_mi.png"

MI_SAMPLE_SIZE = 450_000

def load_data_via_requests(firebase_url, label_col=LABEL_COL, drop_leak=LEAKY_COLUMNS):
    print(f"[INFO] Downloading IoT-23 CSV from {firebase_url} with requests...")
    resp = requests.get(firebase_url, verify=True)
    resp.raise_for_status()

    filelike = io.StringIO(resp.text)
    df = pd.read_csv(filelike, low_memory=True)
    print(f"[INFO] Loaded data: {df.shape[0]} rows, {df.shape[1]} columns from Firebase URL.")
...
def plot_mutual_info(X, y):
    ...
    plt.title("IoT-23: Feature 'Leakage' Score via MI")
    plt.tight_layout()
...
def batch_retraining_demo():
    print("[INFO] No local historical file usage in this remote approach. Doing nothing.")


def main():
    X, y = load_data_via_requests(IOT23_FIREBASE_URL)
    ...
    if len(X) > MI_SAMPLE_SIZE:
        print(f"[INFO] Large dataset: {len(X)} rows. Sampling {MI_SAMPLE_SIZE} for MI.")
        sample_idx = np.random.choice(len(X), MI_SAMPLE_SIZE, replace=False)
    ...
    mi_df = plot_mutual_info(X_sample, y_sample)
    ...
    print(f"\n[INFO] Best model => {best_name}, macro_f1={best_f1:.4f}")
    ...
    data_dict = {
        ...
        "dataset_url":  IOT23_FIREBASE_URL
    }
