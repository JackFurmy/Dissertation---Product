...
def bundle_path() -> Path:
    return MODEL_DIR / "phishing_url.pkl"
...
def train_phish_url_mem2(
    chunk_size : int  = 50_000,
    max_rows   : int  = 150_000,         
    load_existing: bool = False,
    split_models : bool = False,
    user_id      : str  = "phish_mem2_user",
):
    prev_models = {}
    ...
    for url in [PHISHING_URL, LINUX_MEMORY2_URL]:
        for x, y, feats in stream_rows(url, chunk_size, label_encoders, feats):
            rows += 1
            ...
            if rows % 50_000 == 0 or rows == max_rows:
                print(f"[INFO] processed {rows:,} rows total …")
            if rows >= max_rows:         
    ...
    joblib.dump(
        {
            ...
            "best_name":    best_name,
            "best_f1":      best_f1,
            "dataset_urls": [PHISHING_URL, LINUX_MEMORY2_URL],
            "rows_trained": rows,
        },
        bundle_path(),
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--max-rows",   type=int, default=150_000,
                    help="Early‑stop after this many rows (default 150 000)")
    ...
    ap.add_argument("--user-id",       default="phish_mem2_user")
    args = ap.parse_args()

    train_phish_url_mem2(
