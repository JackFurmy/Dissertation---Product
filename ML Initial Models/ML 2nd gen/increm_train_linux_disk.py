...
MODEL_DIR = Path(
    "/USER_FILEPATH/cybersecurity_game/game/game/models/models"
)
...
def stream_rows(
    ...
    for url in url_list:
        ...
        df = (
            pd.read_csv(io.StringIO(requests.get(url, verify=True).text), low_memory=True)
            .dropna(subset=[label])
            .drop_duplicates()
        )
...
def bundle_path() -> Path:
    return MODEL_DIR / "linux_disk.pkl"
...
def train_linux_disk(
    chunk_size: int = 50_000,
    load_existing: bool = False,
    split_models: bool = False,
    user_id: str = "linux_disk_user",
):
    prev_models = {}
    if load_existing and bundle_path().exists():
        ...
        print(f"continuing from bundle ⇒ {bundle_path()}")
    ...
    for x_raw, y in stream_rows(LINUX_DISK_URLS, chunk_size, label_encoders):
        rows += 1
...
if __name__ == "__main__":
    ...
    ap.add_argument("--user-id",       default="linux_disk_user")
    args = ap.parse_args()
